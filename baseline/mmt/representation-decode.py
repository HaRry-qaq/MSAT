...
def main():
    ...
    OutputPath = "/work100/weixp/mtmt3-paper-2/pre-dataset/mtmt/test"
    datanames = os.listdir(InputPath)
    for i in datanames:
        print(i)
        data_path = InputPath + '/' + i
        save_path =  i[:-4]+'.mid'
        with open(data_path,'rt') as csvfile:
            ...
            # print(notes)
            # Extract instruments
            print(program_instrument_map[48])
            instruments = set(program_instrument_map[note[-1]] for note in notes)
            ...
            # for type, beat, position, pitch, duration, program in notes:
            for  type, beat, position, pitch, duration, program in notes:
                # Skip if max_beat has reached
                if beat > max_beat:
                    continue
            ...
            # sample_dir = pathlib.Path(f"/freeneb/home/weixp/tst/MIDI-VAE/loss_5/16")
            # music.write(sample_dir / "baseline-midi" / save_path)
            music.write(OutputPath+'/'+save_path)
            print("-" * 40)
